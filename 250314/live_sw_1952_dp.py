...
for tc in range(1, T+1):
    ...
    dp[1] = min(days[1] * day, month1)
    # 2월의 가격 = 1월의 가격 + (1일권 구매 vs 1달권 구매)
    dp[2] = dp[1] + min(days[2] * day, month1)
    ...
    for month in range(3, 13):
        ...
        dp[month] = min(dp[month-3] + month3, dp[month-1]+(days[month]*(day)), dp[month-1]+month1)

    # 12월의 누적 최소 금액 vs 1년 권
