...
def article_detail(request, article_pk):
    ...
    # 단일 게시글 데이터 조회 + 그 단일 게시글에 작성된 댓글의 개수도 계산해달라고 DB에 요청
    article = Article.objects.annotate(num_of_comments=Count('comment')).get(pk=article_pk)
    # 기존 article 객체에는 없었지만 결과에만 잠시 포함된 데이터
    # 실제 article 테이블의 컬럼이 변한 것은 아니다
...
def comment_create(request, article_pk):
    # 어떤 게시글에 작성되는 댓글인지, 단일 게시글 조회 
    article = Article.objects.get(pk=article_pk)
    # 사용자가 보낸 댓글 데이터를 활용해 가공
    serializer = CommentSerializer(data=request.data)
    # 유효한지 검사
    if serializer.is_valid(raise_exception=True):
        # 추가 데이터를 save의 인자로 작성 
