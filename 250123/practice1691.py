...
def is_validation(user_data2):
    False_group = []
    if user_data2['blood_group'] not in blood_types:
        False_group.append('blood_group')
    if user_data2['company'] in black_list:
        return 'blocked', []
    if '@' not in user_data2['mail']:
        False_group.append('mail')
    if 2 > len(user_data2['name']) <= 30:
        False_group.append('name')
    if len(user_data2['website']) < 1:
        False_group.append('website')
    if len(False_group) == 0:
        return True, []
    elif len(False_group) == 1:
        return False, False_group[0]
    else: 
        return False, False_group
